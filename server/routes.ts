...
import { redactEvaluator } from "./lib/redactEnv";
...
export async function registerRoutes(
  ...
  app.get("/api/evaluators", async (_req, res) => {
    ...
    res.json(list.map(redactEvaluator));
  });
  ...
  app.post("/api/evaluators", async (req, res) => {
    ...
    res.status(201).json(redactEvaluator(evaluator));
  });
  ...
  app.patch("/api/evaluators/:id", async (req, res) => {
    ...
    res.json(updated ? redactEvaluator(updated) : updated);
  });
