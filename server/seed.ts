...
function loadBenchmarkData(): BenchmarkData {
  ...
  if (!raw) {
    throw new Error(
      `benchmark.json is empty at ${dataPath}. Run: node server/scripts/generate_benchmark_json.mjs`,
    );
  }
  const parsed = JSON.parse(raw) as BenchmarkData & { _comment?: string };
  if (!parsed.models?.length || !parsed.taskDefinitions?.length) {
    throw new Error("benchmark.json must include non-empty models and taskDefinitions arrays");
  }
  return parsed;
}
